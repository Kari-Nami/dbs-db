...
-- ------------------------------------------------------------------------------------------
-- Table 7: build_parts (junction table)
-- Links builds to their constituent parts with quantities
-- ------------------------------------------------------------------------------------------
...
-- Requests from users for builders to assemble their builds
-- ------------------------------------------------------------------------------------------
...
-- Inquiries from users interested in showcase builds
-- ------------------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------------------
-- Table 13: likes
-- User likes on builds (one per user per build)
-- ------------------------------------------------------------------------------------------
CREATE TABLE likes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
SELECT * FROM part_categories ORDER BY sort_order
...
DELETE FROM builds WHERE id = $1 RETURNING user_id;
...
-- Purpose: Delete like and decrement build's like_count
-- Used by: builds.js route
...
WHERE br.status = $1  -- optional: 'open', 'claimed', etc. (omit for all)
...
-- Used by: BuildDetailPage.jsx
-- Note: Minimal fields - just need to know if one exists and basic status
SELECT 
  br.id,
  br.status,
  br.user_id
...
-- Purpose: Requester accepts an offer, it then cascade to rejects all others, marks request as claimed
...
SELECT id, email, display_name, avatar_url, bio, role, is_banned, created_at, updated_at 
FROM users 
...
UPDATE builder_applications SET status='rejected', admin_notes=$1, reviewed_by=$2, reviewed_at=now()
...
-- Query 65: Get platform statistics (Homepage)
...
-- -- ------------------------------------------------------------------------------------------
-- -- Users (5 accounts: 1 admin, 2 regular users, 2 builders)
-- -- ------------------------------------------------------------------------------------------
...
-- -- ------------------------------------------------------------------------------------------
-- -- Builder Profiles (2 verified builders)
-- -- ------------------------------------------------------------------------------------------
...
-- -- ------------------------------------------------------------------------------------------
-- -- Build Parts (linking builds to parts)
-- -- ------------------------------------------------------------------------------------------
...
-- -- ------------------------------------------------------------------------------------------
-- -- Comments (4 sample comments with threading)
-- -- ------------------------------------------------------------------------------------------
...
-- -- ------------------------------------------------------------------------------------------
-- -- Compatibility Rules (12 rules for validating PC part compatibility)
-- -- ------------------------------------------------------------------------------------------
